...
if [[ ${ALL_DATABASES} == "" ]]; then
	...
	mysqldump $DSN --all-databases --compress --flush-logs --flush-privileges --master-data=2 --routines --single-transaction --dump-date --log-error=/mysqldump/${DB_HOST}/$(date +'%Y%m%d%H')/mysqldump.log.err |gzip > /mysqldump/${DB_HOST}/$(date +'%Y%m%d%H')/${DB_HOST}.consistent.dump.gz 
